...
public class HashMapDefault<K, V> implements Map<K, V> {
	private static final boolean RED = false;
	private static final boolean BLACK = true;
	private int size;
	private Node<K, V>[] table;

	private static final int DEFAUT_CAPACITY = 1 << 4;

	public HashMapDefault() {// 1<<4 == 2^4
		table = new Node[DEFAUT_CAPACITY];
	}
...
////			String k1ClString = k1.getClass().getName();
////			String k2ClString = k2.getClass().getName();
////			result = k1ClString.compareTo(k2ClString);
////			if (result != 0) {
////				return result;
////			}
//
	...
	private Node<K, V> node(Node<K, V> node, K k1) {
		...
		while (node != null) {
			...
			if (h1 > h2) {
				...
				return node;
			} else if (k1 != null && k1 != null && k1.getClass() == k2.getClass() && k1 instanceof Comparable
					&& (cmp = ((Comparable) k1).compareTo(k2)) != 0) {
				node = cmp > 0 ? node.right : node.left;
			} else // 哈希值相等,不具备可比较性也不equals
